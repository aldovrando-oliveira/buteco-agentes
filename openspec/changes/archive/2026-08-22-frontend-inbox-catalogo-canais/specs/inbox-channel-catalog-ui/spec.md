...
canal consumindo `GET /channels/{id}`, exibindo nome, tipo, agente
responsável, estado, datas de criação/atualização, a URL de webhook
(`webhookUrl`) em destaque com um botão de copiar e um texto de instrução
que varia conforme o `ChannelType`, e ações para editar, ativar/desativar
o canal. A credencial nunca é exibida, pois `ChannelResponse` nunca a
inclui.
...
- **WHEN** o usuário acessa a página de detalhe de um canal com
  `ChannelType: waha`
