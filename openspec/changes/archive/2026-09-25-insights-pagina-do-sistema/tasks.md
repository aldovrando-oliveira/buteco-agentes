...
- [x] 0.1 **Conferir que a #65 fechou** — a série diária precisa emitir `0` para
      ...
      **Conferido em 24/09 contra a rota real**, com `apps/api` local e
      `TZ=America/Sao_Paulo`, pedindo uma janela que começa antes do início do
      regime: `dailySeries` traz 22, 23 e 24/09 com `taskCount: 0` e
      `tokenCount` nulo, nenhum dia anterior a 22/09 aparece, `byWeekday` traz
      só `2`, `3` e `4`, e `peakWeekday` é nulo. Os quatro pontos conferem; a
      releitura está no `design.md`.
- [x] 0.2 Confirmar que o XML doc de `DailyInsightPoint` foi corrigido junto —
      é parte do escopo da #65, e é o que impede a próxima leitura de repetir o
      engano. **Conferido:** o XML doc afirma que a ausência de um dia significa
      uma coisa só, e registra por escrito que o texto anterior dizia mais do
      que a consulta fazia (convenção 9).
...
- [x] 1.1 Criar `src/features/insights/types/systemInsights.ts` com o contrato da
      ...
      desserialização.
- [x] 1.2 Criar `src/features/insights/api/insightsApi.ts` com `request<T>` e
...
- [x] 1.3 Acrescentar `getSystemInsights(from, to)` montando a query string com
      os **dois** limites obrigatórios, no idioma de `periodQuery` de
      `sessionsApi.ts`.
- [x] 1.4 Escrever `insightsApi.test.ts`: a URL montada com os dois limites; o
      `401` limpando o token; o `ValidationProblem` de janela chegando como
      `ApiError` com o corpo preservado.
- [x] 1.5 Criar `src/features/insights/utils/insightsWindow.ts` — período
...
- [x] 1.6 Escrever `insightsWindow.test.ts`: os três períodos; "agora" como
      parâmetro produzindo janela determinística; saída sempre em UTC com `Z`.
- [x] 1.7 Criar `src/features/insights/api/useSystemInsights.ts` — `useQuery` com
      ...
      `useSessions.ts`.
- [x] 1.8 Escrever `useSystemInsights.test.ts`: uma consulta por período; renders
      repetidos sem troca de período não geram consulta nova; nova tentativa
      recalcula a janela.
- [x] 1.9 Criar `src/features/insights/test/systemInsightsFixture.ts` — construtor
...
- [x] 2.1 Criar `utils/metricState.ts`: dado `number | null | undefined` mais o
      ...
      em nenhum caminho.
- [x] 2.2 Escrever `metricState.test.ts` com os guardas **negativos**: nulo e
...
- [x] 2.3 Criar `utils/measuredDays.ts`: janela + `dailySeries` + fuso da
...
- [x] 2.4 Escrever `measuredDays.test.ts`: dia presente com contagem; dia
...
- [x] 2.5 Acrescentar a `measuredDays.ts` a cobertura por dia da semana: quais
...
- [x] 2.6 Escrever o caso correspondente: série de dois dias cobre dois dias da
      semana, e os outros cinco saem como não medidos — **nenhum** deles como
      zero.
- [x] 2.7 Criar `utils/heatScale.ts`: quantização linear em cinco faixas sobre
      `[1, máximo medido]`, com passo próprio para o zero medido. Função pura.
- [x] 2.8 Escrever `heatScale.test.ts`: série normal distribui nas cinco faixas;
      série de valor único não quebra; máximo 1 não divide por zero; o zero
      medido nunca recebe passo de intensidade.
- [x] 2.9 Criar `utils/failurePhaseLabels.ts` — as sete fases de
...
- [x] 2.10 Escrever `failurePhaseLabels.test.ts`: cada fase conhecida; e o
      desconhecido devolvendo o valor cru, **não** o rótulo de outra fase.
- [x] 2.11 Criar `utils/caveatLabels.ts` — o mapa **fechado** dos cinco códigos
      ...
      visível.
- [x] 2.12 Escrever `caveatLabels.test.ts`: os cinco códigos; o desconhecido
...
- [x] 3.1 Acrescentar `--buteco-heat-0` a `--buteco-heat-5` ao
...
- [x] 3.2 Acrescentar a `src/theme.test.ts` as asserções de contrato: os seis
...
- [x] 4.1 `components/MetricValue.tsx` — renderiza os quatro estados de
      `metricState`, com o travessão sempre acompanhado da razão.
- [x] 4.2 `MetricValue.test.tsx`, com os guardas negativos: para origem nula,
      afirmar que o texto renderizado **não** contém `0`.
- [x] 4.3 `components/DeclaredGap.tsx` — o estado de lacuna declarada do quadro 6
      de `Estados.dc.html`: moldura tracejada, o nome do que falta e o texto do
      motivo. Apresentação **distinta** do travessão.
- [x] 4.4 `DeclaredGap.test.tsx`: renderiza o nome e o motivo; e o negativo —
      **não** renderiza número nem travessão.
- [x] 4.5 `components/PeriodPicker.tsx` — 7d / 30d / 90d, com 30d como padrão.
- [x] 4.6 `PeriodPicker.test.tsx`: aciona o callback com o período escolhido; o
      selecionado é o único marcado.
- [x] 4.7 `components/InsightsKpiGrid.tsx` — os seis cards do protótipo, na ordem
...
- [x] 4.8 `InsightsKpiGrid.test.tsx`: cada card com a sua fonte; o total de
...
- [x] 4.16 `ProviderConsumptionCard.test.tsx`: célula vazia sem `0`; total com
      uma parcela nula; total com todas nulas ficando vazio; o texto presente.
- [x] 4.17 `components/ConversationModelsCard.tsx` — Modelo / Provedor /
      ...
      Cache lido (L3).
- [x] 4.18 `ConversationModelsCard.test.tsx`: a ordenação; `totalTokens` nulo
...
- [x] 4.19 `components/AgentConsumptionCard.tsx` — Agente / Tokens / Falhas, com
...
- [x] 4.20 `AgentConsumptionCard.test.tsx`: nome cruzado; agente fora do catálogo
      ...
      aparece preenchida.
- [x] 4.21 `components/FailuresCard.tsx` — as duas contagens separadas, o texto
...
- [x] 4.22 `FailuresCard.test.tsx`: as duas contagens; a falha com percentual; a
      recusa **sem** percentual e com o caveat; total de tasks zero não
      produzindo percentual nem `NaN`.
- [x] 4.23 `components/FailureReasonsCard.tsx` — fases de falha e falhas de
...
- [x] 4.24 `FailureReasonsCard.test.tsx`: cada fase com o seu rótulo; a
...
- [x] 4.25 `components/NonTerminalBanner.tsx` — as duas populações separadas com
...
- [x] 4.26 `NonTerminalBanner.test.tsx`: as duas populações; o texto do instante;
...
- [x] 5.1 Criar `pages/SystemInsightsPage.tsx`: detém `useSystemInsights` e
...
- [x] 5.2 Montar o cabeçalho: a janela pedida em datas locais, e o "medindo
      desde" **por regime**, junto do grupo que cada regime mede — nunca um texto
      único para a página.
- [x] 5.3 Escrever `SystemInsightsPage.test.tsx`: uma requisição à rota agregada
...
- [x] 5.4 Acrescentar a rota `insights` a `src/app/routes.tsx`, dentro do grupo
      protegido e da casca.
- [x] 5.5 Acrescentar o item `Insights` a `navItems` em
      `src/components/layout/AppShell.tsx`, com ícone do `lucide-react`.
- [x] 5.6 Acrescentar os casos a `router.test.tsx` e `AppShell.test.tsx`: a rota
      resolve para a página; o item aparece com rótulo e ícone e fica ativo na
      rota.

## 6. Verificação de suíte

- [x] 6.1 Rodar `npm test -- --run` em `apps/frontend` e comparar com a baseline
      do `design.md` (**927/927** em 84 arquivos, remedida em 24/09/2026 sobre
      `4c33eba`). Comparar **por nome de teste**, nunca só pelo total.
- [x] 6.2 Rodar `npm run lint` e a checagem de tipos de `apps/frontend`.
- [x] 6.3 Confirmar que as suítes de `apps/api` (**404/404**, remedida em 24/09
      — a de 391 é anterior à #65), `apps/workers`
...
> **DESTRAVADO em 24/09**: o dono logou na aba, e o banco de dev foi semeado
> (`task_executions` + `provider_calls` em 22 e 24/09, com 23/09 deixado vazio
> de propósito para ser o dia **medido e vazio**, e `google` sem reportar token
> nenhum para exercitar a célula vazia e o par vazio × zero na mesma linha).
> A semente é dado de exemplo, não medição, e o script está no diretório
> temporário do job — **não** entra no repositório.
>
> **A conferência pegou um defeito de correção que a suíte verde não pegava** —
> três estados de consulta modelados como dois, fazendo a página afirmar
> "Tasks executadas: 0" com a API fora do ar. Corrigido, com guarda que reprovou
> antes de valer. Registro completo no `02`.

- [x] 7.1 Subir o painel contra o piloto e capturar a página em **7d, 30d e 90d**,
      nos **dois esquemas** de cor — seis capturas.
- [x] 7.2 Capturar a **inversão da escala do mapa de calor** lado a lado: a suíte
      afirma que as variáveis existem, não que a rampa cresce no sentido certo.
- [x] 7.3 Capturar o contraste entre a faixa hachurada e a célula de zero medido
      **no tema claro**, que é onde a distinção é mais fácil de perder.
- [x] 7.4 Produzir os seis quadros de `Estados.dc.html` contra a tela real:
...
- [x] 7.5 Capturar as duas tabelas com nome de agente e de modelo longos — a
      quebra de linha, que jsdom não calcula.
- [x] 7.6 Capturar os cinco textos de caveat em largura de trabalho real (~1860px).
- [x] 7.7 Entregar as capturas ao dono e **aguardar o julgamento**. Cada correção
      muda o que fica visível: repetir 7.1 a 7.6 depois de qualquer ajuste visual.
      **Cumprida em TREZE rodadas**, entre 24 e 25/09, e o ciclo previsto aqui
      foi exercido em cada uma: o dono apontou, o agente mediu no artboard,
      corrigiu e escreveu guarda. O registro completo está no `02`, rodada a
      rodada, com os onze eixos que a suíte não cobria. **Nenhum dos achados do
      dono foi de lógica** — todos de fidelidade ao protótipo, de registro ou de
      contraste —, e é a medida mais limpa da divisão que a convenção 14
      escreve.

## 8. Registro e fechamento

- [x] 8.1 Escrever o `02-HISTORICO_E_STATUS.md` com os itens abertos que esta
...
- [x] 8.2 Registrar os **dois gatilhos já cumpridos** por esta change: a resposta
      ...
      nomeou sem resolver.
- [x] 8.3 Fechar a **décima nona medição da convenção 18**: comparar a projeção
...
- [x] 8.4 Atualizar o `CHANGELOG.md` e rodar `scripts/check-docs.py`.
- [ ] 8.5 **(pós-archive, pela convenção 24)** Abrir o PR referenciando a **#52**
      no corpo, e conferir que o
      `proposal.md` abre com `**Issue:** #52` (convenção 23).
- [ ] 8.6 **(pós-archive, pela convenção 24)** Registrar na **#52** o fechamento:
      as capturas da conferência manual,
      o resultado da décima nona medição, e o estado de cada uma das cinco
