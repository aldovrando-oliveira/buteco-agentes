...
mensagem normalizada `(ChannelId, ExternalId, texto, receivedAt)`, resolve
`Contact`/`Session` usando o resolvedor já existente, e adiciona a mensagem
ao buffer de debounce da sessão resolvida — sem exigir nenhum endpoint
HTTP para essa ingestão.

#### Scenario: Mensagem recebida é associada à Session resolvida
- **WHEN** o serviço interno é chamado com um `(ChannelId, ExternalId)` e um
  texto de mensagem
- **THEN** a `Session` correspondente é resolvida (criada ou reaproveitada,
  conforme as regras já existentes de resolução de sessão) e a mensagem é
  adicionada ao buffer de debounce dessa `Session`
