# knowledge-embedding-metrics Specification

## Purpose

Cobre a **coleta durável do consumo de embedding**: uma linha por chamada ao
gateway — nas duas finalidades, indexação de documento e busca em base de
conhecimento —, e uma linha por tentativa de indexação, com o desfecho e a fase
em que ela parou. É a fonte de **M19** (tokens de embedding) e de **M30**
(falhas de indexação), que até aqui não tinham nenhuma: `knowledge_documents`
guarda o estado de indexação como **corrente**, sobrescrito a cada tentativa, e
uma tentativa que falhou e depois deu certo não deixava rastro.

O grão da linha de chamada é a **requisição**, e como a geração de embedding é
loteada isso quer dizer **uma linha por lote** — é o que torna atribuível a
falha de um lote específico. Escrita por `apps/workers`, em tabelas migradas por
`apps/api`, com nulo preservado (nulo é "o provedor não reportou", zero é "o
provedor reportou zero") e **sem efeito sobre o desfecho da indexação nem sobre
o resultado que a busca devolve ao modelo**.
