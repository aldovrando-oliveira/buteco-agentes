# inbox-message-period-summary Specification

## Purpose

Cobre a **contagem agregada de mensagens de entrada (`Inbound`) de `apps/inbox`
por intervalo de datas**, servida como recurso próprio de leitura em
`GET /messages/summary`: os limites do intervalo, sua validação, e o instante que
decide a inclusão de cada mensagem — o `OccurredAt`, que é o instante de
recebimento pelo servidor. A unidade contada é a **mensagem distinta**, nunca a
entrega de webhook que a trouxe.
