...
TBD - defined by change dedupe-global-nome-de-tool. Update Purpose after
archive. Cobre o namespace compartilhado dos nomes de tool entregues ao LLM
em `apps/workers`: a unicidade dos nomes no conjunto final que une as tools
MCP resolvidas com as tools de delegação resolvidas, a precedência entre os
conjuntos de origem na resolução de colisões, o formato e o limite de
tamanho do nome exposto, a observabilidade da renomeação por colisão, e a
estabilidade do conjunto de nomes entre execuções.
