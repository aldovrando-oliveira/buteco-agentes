...
- **WHEN** o browser faz uma navegação real (`Sec-Fetch-Mode: navigate`)
  ...
  prefixo de API (`/agents/{id}`, `/channels/{id}` ou
  `/mcp-servers/{id}`)
