...
| `apps/frontend` | UI de gestão (agentes, MCP, delegação, canais), tela de login | React 19.2+, TypeScript, Vite, Mantine v9, `react-router` v8, `@tanstack/react-query` v5, `react-markdown`+`remark-gfm`, Vitest+Testing Library | — |
| `apps/inbox` | Catálogo de canais de entrada, CRM (Contact/Session), orquestrador de debounce, adapters de canal (WAHA, Telegram) | .NET 10 Minimal API, CQRS próprio (Mediator), EF Core | Postgres **próprio** (`buteco_inbox`), isolado — sem tabela em comum com `apps/api`/`apps/workers` |
...
fronteira por **inatividade automática** (timeout configurável) — sem
encerramento explícito (aberto, ver histórico).
...
contra `apps/api`. Concorrência otimista via `xmin` do Postgres.
...
> instância de `DbContext`). Corrigido com detach+rebusca. Qualquer
> mudança que encoste nessa coleção, ou que re-leia a entidade depois de
> mutá-la, precisa de teste com concorrência de verdade — não caminho
> feliz.
...
4. **Degradação graciosa**: falha de dependência externa (MCP, alvo de
   delegação, entrega de webhook) nunca derruba a task principal — é
   logada, não propagada.
5. **Testes de integração com infraestrutura real** — Testcontainers
   ...
   caso "com item" ganha o par "sem item"/vazio testado. Quando o risco é
   dois processos concordarem sobre alguma coisa, o teste usa o artefato
   real produzido por um contra o outro — não um artefato forjado no
   teste com a mesma configuração.
...
8. **Checagem de integridade no startup** é o padrão para todo registro
   ...
   aplicado a dois casos de natureza diferente: DI keyed para pontos de
   extensão tipo-plugin (contrato de canal) e classificação de rotas
   anônimas (autenticação). A checagem vale nos **dois sentidos** quando
