...
/// — a união das tools MCP com as tools de delegação.
/// </summary>
/// <remarks>
/// <para>
/// Mora no ponto de concatenação, e não dentro de cada resolvedor, porque é o
/// único ponto do sistema que sabe que os dois conjuntos dividem namespace
/// (design.md da change dedupe-global-nome-de-tool, Decisão 1). Cada resolvedor,
...
{
    /// <summary>
    /// Une os dois conjuntos resolvendo colisões de nome. A ordem dos parâmetros
    /// é a precedência declarada (Decisão 5): tool MCP mantém o nome pretendido,
    /// tool de delegação é a renomeada. Não é consequência da ordem de dois
    /// operandos de <c>Concat</c>, como era antes desta change.
    /// </summary>
        ...
        IReadOnlyList<AITool> delegationTools)
    {
        ...
        var deduplicated = new List<AITool>(mcpTools.Count + delegationTools.Count);
    ...
    private static AITool Rename(AITool tool, string finalName) => tool switch
    {
        ...
        // Nenhuma tool destes dois conjuntos cai aqui hoje (os dois lados
