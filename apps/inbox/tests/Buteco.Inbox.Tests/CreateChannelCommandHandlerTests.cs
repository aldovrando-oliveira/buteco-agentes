...
public class CreateChannelCommandHandlerTests
{
    ...
    public async Task Handle_AgentFound_PersistsChannelWithEncryptedCredentials()
    {
        ...
        var handler = new CreateChannelCommandHandler(dbContext, cipher, validatorMock.Object);
        var agentId = Guid.NewGuid();

        var command = new CreateChannelCommand(ChannelType.WhatsApp, "Canal de Suporte", "token-secreto", agentId);
    ...
    public async Task Handle_AgentNotFound_DoesNotPersist()
    {
        ...
        var handler = new CreateChannelCommandHandler(dbContext, CreateCipher(), validatorMock.Object);

        var command = new CreateChannelCommand(ChannelType.WhatsApp, "Canal de Suporte", "token-secreto", Guid.NewGuid());
        var result = await handler.Handle(command, CancellationToken.None);
    ...
    public async Task Handle_AgentValidationCommunicationFailure_DoesNotPersist()
    {
        ...
        var command = new CreateChannelCommand(ChannelType.Telegram, "Canal de Vendas", "token-secreto", Guid.NewGuid());
        var result = await handler.Handle(command, CancellationToken.None);
    ...
    public async Task Handle_AgentFoundButInactive_StillPersistsChannel()
    {
        ...
        var command = new CreateChannelCommand(ChannelType.WhatsApp, "Canal Com Agente Inativo", "token-secreto", Guid.NewGuid());
        var result = await handler.Handle(command, CancellationToken.None);
